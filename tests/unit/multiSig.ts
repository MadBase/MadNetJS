...
describe("Unit/MultiSig:", () => {
    ...
    before(async function () {
        ...
        secondaryPrivateKey =
            process.env.OPTIONAL_TEST_SUITE_SECONDARY_PRIVATE_KEY;
        msgHex = Buffer.from("hello world", "utf8")
            .toString("hex")
            .toLowerCase();
        ...
        publicKeys = [
            await bnAccount.signer.getPubK(),
            await bnAccount2.signer.getPubK(),
        ];

        let multiAccount = await madWallet.account.addMultiSig(publicKeys);
        let multiPubK = await multiAccount.signer.getPubK();

        let signatureOne = await bnAccount.signer.multiSig.sign(
            msgHex,
            multiPubK
        );
        let signatureTwo = await bnAccount2.signer.multiSig.sign(
            msgHex,
            multiPubK
        );
    ...
    describe("Public Key and Address", () => {
        it("Fail: Reject getPubK when no Public Key is added", async () => {
            await expect(multiSigBn.getPubK()).to.eventually.be.rejectedWith(
                "Need public keys"
            );
        });

        it("Fail: Reject getAddress when no Public Key is added", async () => {
            await expect(multiSigBn.getAddress()).to.eventually.be.rejectedWith(
                "Need public keys"
            );
        });
        ...
        it("Success: Get Address when a Public Key exists", async () => {
            ...
            await expect(multiSigBn.getAddress())
                .to.eventually.match(/^[0-9a-fA-F]+$/)
                .and.have.length(40);
        });
    });
    ...
    describe("Sign and Multi Sign", () => {
        it("Fail: Reject Sign when called with invalid rawMsg", async () => {
            await expect(multiSigBn.sign(null)).to.eventually.be.rejectedWith(
                "Missing input"
            );
        });
        ...
        it("Fail: Reject Sign Multi when called with invalid rawMsgs", async () => {
            await multiSigBn.addPublicKeys(publicKeys);
            await expect(multiSigBn.sign("0000ffeebabe")).to.eventually.be
                .fulfilled;
        });
        ...
        it("Success: Sign Multi messages", async () => {
            await multiSigBn.addPublicKeys(publicKeys);
            await expect(
                multiSigBn.signMulti(["0000ffeebabe", "0000ffeebabe"])
    ...
    describe("Aggregate and Verify Signatures", () => {
        ...
        it("Fail: Aggregate Signatures should fail unless an array is provided ", async () => {
            await expect(
                ...
                "Call using map[string]interface {} as type []interface {}"
            );
        });

        it("Success: Aggregate Signatures", async () => {
            await expect(multiSigBn.aggregateSignatures(signatures)).to
                .eventually.be.fulfilled;
        });

        it("Success: Aggregate Multi Signatures", async () => {
            await expect(multiSigBn.aggregateSignaturesMulti([signatures])).to
                .eventually.be.fulfilled;
        });
        ...
        it("Success: Verify Aggregate Single", async () => {
            ...
            const verifiedSig = await multiSigBn.verifyAggregateSingle(
                msgHex,
                pubKey,
                sig
            );
        ...
        it("Success: Verify BN Signature", async () => {
            const accountBN = await madWallet.account.getAccount(
                bnAccount.address
            );
        ...
        it("Success: Sign Multiple Messages", async () => {
            ...
            const signatures = await accountBN.signer.signMulti([
                msgHex,
                msgHex,
            ]);
